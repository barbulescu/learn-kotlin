...
fun groupByLength(words: List<String>): Map<Int, List<String>> = words.groupBy { it.length }

// Split numbers into (evens, odds). First element of the Pair is the evens.
// partitionEvenOdd([1,2,3,4,5]) → Pair([2,4], [1,3,5])
fun partitionEvenOdd(numbers: List<Int>): Pair<List<Int>, List<Int>> = numbers.partition { it % 2 == 0 }

// Split each sentence by spaces and return all words in a single flat list.
// allWords(["hello world", "foo bar"]) → ["hello", "world", "foo", "bar"]
fun allWords(sentences: List<String>): List<String> = sentences.flatMap { it.split(" ") }

// Pair up elements from two lists by index.
// zipPairs(["a","b","c"], [1,2,3]) → [("a",1), ("b",2), ("c",3)]
fun zipPairs(keys: List<String>, values: List<Int>): List<Pair<String, Int>> = keys.zip(values)
